...
在Jekyll中，每篇博客都是一个独立的在`_post`中以`.md`或者`.markdown`为后缀的文件，使用[markdown](http://wowubuntu.com/markdown/)书写。这里记录一些常用基本语法以便可以随时查看。
...
*   代码块不能正常显示。
    ...
    MarkDown解析器kramdown默认使用代码高亮coderay，可是Github却不支持。去掉coderay，并修改相应的css解决。
*   模板中的估计阅读时间不正确。
